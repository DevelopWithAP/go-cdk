...
func(u DynamoDBClient) InsertUser(user types.User) error {
	// assemble item in a dynamodb compatible format
...
func (u DynamoDBClient) GetUser (username string) (types.User, error) {
	...
	result, error := u.databaseStore.GetItem(&dynamodb.GetItemInput{
		TableName: aws.String(TABLE_NAME),
		Key: map[string]*dynamodb.AttributeValue{
			"username": {
				S: aws.String(username),
	...
	error = dynamodbattribute.UnmarshalMap(result.Item, &user)
	if error != nil {
		return user, error
	}
